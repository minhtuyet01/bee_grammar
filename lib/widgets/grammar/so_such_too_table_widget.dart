...
class SoSuchTooTableWidget extends StatelessWidget {
  const SoSuchTooTableWidget({super.key});
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildDataRow(
            'So + tính từ/trạng từ',
            'So + adj/adv + that + S + V\nVí dụ: He is so tall that he can touch the ceiling.\n(Anh ấy cao đến nỗi có thể chạm trần)',
          ),
          Divider(height: 1, color: Colors.grey.shade300),
          _buildDataRow(
            'Such + danh từ',
            'Such + (a/an) + adj + N + that + S + V\nVí dụ: It was such a good film that I watched it twice.\n(Đó là một bộ phim hay đến nỗi tôi xem 2 lần)',
          ),
          Divider(height: 1, color: Colors.grey.shade300),
          _buildDataRow(
            'Too + tính từ',
            'Too + adj/adv + to V\nVí dụ: He is too young to drive.\n(Anh ấy quá trẻ để lái xe)',
          ),
        ],
      ),
    );
